...
class lcpt_posttype_list_table extends WP_List_Table{
    ...
    function column_cpt_name($item){
        ...
        $actions = array(
            'view' => sprintf('<a href="?page=%s&action=lcpt_edit&id=%s">%s</a>',  $_REQUEST['page'],$item[$this->tbl_unikid], __('Edit', 'lcpt_table')),
            'delete' => sprintf('<a href="?page=%s&action=lcpt_delete&id=%s">%s</a>', $_REQUEST['page'], $item[$this->tbl_unikid], __('Delete', 'lcpt_table')),		
			$status => sprintf('<a href="?page=%s&action=lcpt_statusupdate&stype=%s&id=%s">%s</a>', $_REQUEST['page'],$item['enabled'], $item[$this->tbl_unikid], __($status, 'lcpt_table')),		
        );
	...
	function column_enabled($item){
        return '<a href="?page='.$_REQUEST['page'].'&action=lcpt_statusupdate&stype='.$item['enabled'].'&id='.$item[$this->tbl_unikid].'"><img src="' . plugins_url( 'images/icon-'.$item['enabled'].'.png' , __FILE__ ) . '" ></a>';
    }#function
